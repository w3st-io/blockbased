...
module.exports = {
	...
	c_readByCatSorted: async (user_id, cat_id, sort = 0, limit, skip) => {
		try {
			// [SANITIZE] //
			sort = parseInt(sort)
			limit = parseInt(limit)
	...
	c_readPinned: async (user_id, cat_id, sort = 0) => {
		try { 
			// [VALIDATE] user_id //
			if (!mongoose.isValidObjectId(user_id)) {
				return {
	...
	// [DELETE] _id & user //
	c_deleteByIdAndUser: async (post_id, user_id) => {
		try {
			// [VALIDATE] user_id //
			if (!mongoose.isValidObjectId(user_id)) {
				return {
